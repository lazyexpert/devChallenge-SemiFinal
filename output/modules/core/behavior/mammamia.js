...
var CONTACT_EMAIL = "nospam@please.com";
...
function formHash(basket) {
  ...
  for (var t = 0; t < basket.internals.length; t++) {
    // Loop thorugh internal properties
    for (var key in basket.internals[t]) {
...
function getBasketFromMenu(type, title, size) {
  var res = menu.filter(function (el) {
    return el.name == type;
  })[0];
  return res.structure.type.filter(function (el) {
...
function fillBasket(order) {
  ...
  items.forEach(function (el) {
    var basketObj = getBasketFromMenu(el.type, el.title, el.size);
    var hash = formHash(basketObj);
    basketObj.hash = hash;
...
function doJob(order) {
  ...
  var basket = JSON.stringify(fillBasket(order));
